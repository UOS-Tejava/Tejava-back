...
public class OrderController {
    ...
    // 주문한 이후, 주문 내역에서 상태가 접수 대기중이 아닐 경우, 옵션이나 스타일 수정 혹은 결제 취소하기
    ...
    public ResponseEntity<?> cancelOrder(@SessionAttribute(name = SessionConst.LOGIN_MEMBER, required = false) User customer, @PathVariable Long order_id) {

        orderService.cancelOrder(customer, order_id);
        return ResponseEntity.ok().build();
    }
    ...
    @GetMapping("/order/showOrderHistory")
    ...
    @ApiOperation(value = "모든 메뉴 보여주기", notes = "회원의 주문 내역을 반환합니다.")
    ...
    @ApiOperation(value = "모든 옵션 보여주기", notes = "회원의 주문 내역을 반환합니다.")
    ...
    @ApiOperation(value = "모든 스타일 보여주기", notes = "회원의 주문 내역을 반환합니다.")
